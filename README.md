...
These apps have extremely locked down permissions and an in-built firewall restricted solely to necessary sites. Third party sign ins (i.e. google, microsoft, etc.) are not supported due to the focus on preventing external analytics. Adjustments via FlatSeal (or manually) can enable features like sound, microphone, and file system access (drag and drop).
